...
public:
    ...
    void add(const Name_Id& in_id, const Box& in_bounds);
...
private:
    ...
    std::unordered_map<uint32, Box> _bounds;
    std::unordered_map<int32, Cell> _cells;

private:
    bool _check_aabb_intersection(const Name_Id& a_id, const Name_Id& b_id);
    void _get_cells_for_bounds(const Box& in_bounds, ivec3& out_min, ivec3& out_max) const;
