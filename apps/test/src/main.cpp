...
struct Game
{
	...
	void initialize()
	{
		...
		Input_System::get().register_event("movement_forward", {
			{"KEY_W", 1.0f},
			{"KEY_S", -1.0f},
			{"KEY_UP", 1.0f},
			{"KEY_DOWN", -1.0f}
		...
		Input_System::get().register_event("movement_right", {
			{"KEY_A", -1.0f},
			{"KEY_D", 1.0f},
			{"KEY_LEFT", -1.0f},
			{"KEY_RIGHT", 1.0f}
...
private:
	void register_inputs()
	{
		Input_System::get().register_event("game_start",
			...
			[this](float value) { if (value > 0.5f) return; if (!game.initialized) game.initialize(); });

		Input_System::get().register_event("game_stop", {{"KEY_ESCAPE", 1.0f}}).bind(
			[this](float value) { if (value > 0.5f) return;  if (game.initialized) game.shutdown(); });
	}
};
